...
export default function AddImballaggioDialog({ open, onClose, onSave, imballaggio }: Props) {
  const imballaggioVuoto = { tipo: '', dimensioni: '', capacitaKg: 0, note: ''};
    ...
    <Dialog open={open} onClose={onClose}>
      ...
      <DialogContent>
        <Stack spacing={2} mt={1}>
          <TextField label="Tipo" value={data.tipo} onChange={(e) => handleChange('tipo', e.target.value)} fullWidth />
          <TextField label="Dimensioni" value={data.dimensioni} onChange={(e) => handleChange('dimensioni', e.target.value)} fullWidth />
          <TextField label="Capacità (Kg)" type="number" value={data.capacitaKg} onChange={(e) => handleChange('capacitaKg', parseFloat(e.target.value))} fullWidth />
          <TextField label="Note" value={data.note} onChange={(e) => handleChange('note', e.target.value)} fullWidth multiline rows={2} />
        </Stack>
      </DialogContent>
      <DialogActions>
